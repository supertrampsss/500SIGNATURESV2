...
def test_seuls_les_exercices_anterieurs_a_la_borne_partent():
    conn = ConnexionSimulee({"commune": ["2020", "2021", "2022", "2023", "2024", "2025"]})
    assert retention.a_supprimer(conn) == {"commune": ["2020", "2021"]}


def test_une_retention_deja_respectee_ne_supprime_rien():
    conn = ConnexionSimulee({"commune": ["2022", "2023", "2024", "2025"]})
...
def test_les_niveaux_non_nommes_ne_sont_jamais_touches():
    ...
    conn = ConnexionSimulee(
        {"commune": ["2022", "2023"], "departement": ["2018", "2019", "2020"]}
    )
...
def test_refus_de_reduire_une_serie_a_moins_de_deux_points():
    ...
    conn = ConnexionSimulee({"commune": ["2020", "2021", "2022"]})
