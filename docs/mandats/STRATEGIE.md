...
```mermaid
flowchart TD
  A[Snapshot et hypothèses versionnés] --> B[État hérité]
  B --> C[Choix du joueur]
  B --> D[Engagements à livrer]
  C --> E[Adaptateur de mode]
  D --> E
  F[Événement et exposition] --> E
  E --> G[Comptes réconciliés]
  E --> H[Indices territoriaux]
  G --> I[Bilan expliqué]
  H --> I
  I --> B
```
...
```mermaid
flowchart TD
  A[Sources publiques autorisées] --> B[Déduplication et exclusions]
  B --> C[Évaluation de pertinence]
  C --> D[Recherche dans contenus approuvés]
  D --> E[Brouillons et preuves]
  E --> F[Revue humaine]
  F --> G[Rejet ou blocage]
  F --> H[Publication manuelle]
  H --> I[Journal et corrections]
  J[Pause et opt-out] --> B
  J --> F
```
...
```mermaid
flowchart TD
  A[Vite multipage] --> B[Site existant]
  A --> C[Mandats]
  A --> D[Méthodologie HTML]
  C --> E[Contrôleur et rendu responsive]
  E --> F[Moteur pur]
  F --> G[Domaine municipal]
  F --> H[Domaine national]
  F --> I[Journal de décisions]
  I --> J[Sauvegarde et replay]
  I --> K[Partage local]
  L[Snapshots validés futurs] --> G
  L --> H
```
