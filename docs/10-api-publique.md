...
| `indicateurs.json` | Catalogue : identifiant, libellé, unité, thème, cadre comptable, sommabilité, définition grand public et technique, formule, badges, `geographie_courante` (le producteur recalcule-t-il son historique dans les communes d'aujourd'hui — si oui, une fusion ne coupe pas la série). `periodes` liste **toute** la série disponible ; `periodes_par_niveau` liste les périodes **cartographiées** — l'historique n'a pas la même profondeur à tous les niveaux, et une carte est bornée aux périodes récentes pour les séries longues. Les séries entières restent dans les fiches de territoire. `jeu` nomme le jeu de données de la série ; `jeu_par_niveau` ne liste que les **mailles dont la source diffère** de `jeu`, et se lit `jeu_par_niveau[niveau] ?? jeu` — un indicateur ne porte qu'un jeu quand une série peut venir d'un jeu par maille, ce qui est le cas des agrégats de l'OFGL, déclarés sous celui des communes et chargés maille par maille. Absent quand il n'y a rien à corriger | 35 ko |
...
| `territoires/<niveau>/index.json` | L'index de la maille, **colonnaire** : un tableau par champ, tous rangés dans l'ordre de `codes` — `noms`, `parents`, `population_municipale` (référentiel INSEE, millésime dans `millesime_geographique`) et `population_reference`, une ligne par territoire et une colonne par période de `periodes`. Ces populations de référence sont celles de l'indicateur nommé par `denominateur` (`ofgl_population_reference`, en `unite` = habitants) : **c'est le dénominateur des montants par habitant, et il change d'un exercice à l'autre** — une dépense de 2022 se rapporte aux habitants de 2022. Un `null` = pas de référence publiée cet exercice-là, on retombe alors sur la population municipale. C'est ce qu'il faut pour peindre une carte et classer un tableau ; les séries, les drapeaux et les événements de périmètre restent dans les lots ci-dessus. Absent des publications antérieures à ce fichier | 1,3 ko (pays), 1,9 ko (region), 9 ko (departement), 2,4 Mo (commune : 34 875 territoires — contre 549 Mo pour les 101 lots) |
...
| `fraicheur.json` | Pour chaque jeu : dernière extraction, retard sur la fréquence annoncée, statut du dernier chargement, contrôles en échec, `valeurs_revisees_90j` (chiffres déjà publiés dont la valeur a changé lors d'un rechargement des 90 derniers jours ; absent des publications antérieures au champ — l'absence ne vaut pas zéro) | 4 ko |
| `references.json` | Ce à quoi se compare un territoire : par indicateur, période et niveau, l'agrégat de l'ensemble — **communes de la région**, puis **communes de France**. Chaque bloc porte `mediane` (montants bruts), `mediane_habitant` (montants ramenés à l'habitant — **une autre grandeur, pas une variante de présentation**), `n`, et pour les montants sommables `total` et `habitants`. **Le repère de comparaison est la médiane**, pas le rapport agrégé : l'ensemble des communes de France dépense 1 246 € par habitant, un rapport écrasé par les grandes villes, quand la commune médiane en dépense 749. La région de référence d'une commune est l'ensemble de ses communes, **jamais le budget du conseil régional** : ce serait comparer deux niveaux de gouvernement | 250 ko |
