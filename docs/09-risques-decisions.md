...
| D6quinquies | Sortir de Supabase : l'entrepôt devient un fichier DuckDB sur R2 | — | **Tranché le 04/08/2026 par le propriétaire.** Constat qui l'a motivé : le site ne lit jamais la base — il lit du JSON servi par le CDN (docs/03 §3). Vérifié : zéro référence à Postgres ou Supabase dans `site/src`. Rien n'interroge donc l'entrepôt hors du pipeline, une fois par jour, pour écrire en masse puis produire des fichiers plats. Ni authentification, ni temps réel, ni requête concurrente, ni latence à tenir : on payait un moteur transactionnel managé pour un traitement par lots, et on en subissait le mode de panne — trente-six heures de données figées (D6quater) parce qu'une base que personne ne lit s'était endormie et que seul le propriétaire pouvait la réveiller. PostGIS n'était qu'un type de colonne : le calcul géométrique se fait en Shapely. Sur 32 tables, 17 n'étaient écrites par aucun module. **Ce qui remplace** : un fichier DuckDB unique dans le bucket R2 qui sert déjà les données publiées, rapatrié au début de chaque job et renvoyé à la fin ; la sauvegarde est le fichier, versionné par le bucket. Plus de plafond à arbitrer — quatorze millions de lignes tiennent pour quelques centimes par mois — donc plus de rétention subie : le garde-fou passe de 470 Mo à 8 Go et redevient un seuil d'invraisemblance, pas un arbitre budgétaire. **Gain non prévu** : les dix-huit tests d'intégration qui exigeaient un Postgres joignable étaient sautés partout sauf dans un job dédié ; ils tournent maintenant partout, en quinze secondes. Ils ont trouvé, à leur première exécution, que la table de passage entre millésimes rendait 0,8 et 0,3 au lieu de 0,75 et 0,25 — le littéral `1.0` étant un DECIMAL(2,1) pour DuckDB, une scission de commune réattribuait sa population de travers, sans erreur, sur toute comparaison inter-millésimes. D6, D6bis, D6ter et D6quater sont clos par cette décision : ils portaient tous sur un plafond qui n'existe plus. |
