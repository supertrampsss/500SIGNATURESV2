...
1. Créer un compte sur https://supabase.com et un projet **plateforme-staging**
   (plan Free, région `eu-west-3` Paris).
2. Dashboard → Database → Extensions : activer **postgis** (et `pg_trgm`).
3. Récupérer la chaîne de connexion (Settings → Database → Connection string,
   mode « URI », utilisateur `postgres`).
4. Appliquer les migrations depuis un poste avec `psql` :
   ...
   (Ou me demander de le faire une fois le secret GitHub créé — voir §3.)
