...
Migrate every retained nonzero V9 option to a separate named carry-forward estimate before it can enter `SCENARIO_V10_CATALOGUE`. A carry-forward entry preserves its V9 delta, annual/once nature and timing; uses the decision's existing institutional `sourceKeys`; records a nonempty scope, a nonnegative base amount and `estimateStatus: "scenario"`; and uses a stable key that never starts with `legacy:`. It must not silently claim the 18 newly audited scopes. Generate no estimate for a strict-null option. The catalogue test iterates every non-null key and proves that it resolves; the registry test proves there are no unreferenced entries and no `legacy:` key. Thus the 18 count above names only the newly audited additions, not the complete registry size.
