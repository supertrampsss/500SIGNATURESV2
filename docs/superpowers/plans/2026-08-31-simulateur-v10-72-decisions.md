...
for (const decision of SCENARIO_V10_CATALOGUE.decisions) for (const option of decision.options) {
  if (option.budgetProfile.estimateKey !== null) {
    assert.equal(hasBudgetEstimate(decision.id, option.id.split(":").at(-1)!, option.budgetProfile.estimateKey), true, option.id);
...
The former estimate is exactly `grossActionMillions: 7_300`, `behavioralOffsetMillions: 730`, `recurringOperatingCostMillions: 0`, `runRateMillions: 6_570`; the latter is exactly `2_275`, `228`, `0`, `2_047`. Export the ordered `STRUCTURAL_ADOPT_DECISION_IDS` list of the 18 audited IDs solely to calculate the 21 689 M€ structural subtotal. Register the 15 nonzero structural `adopt` estimates with their exact base, gross, behavior, operating-cost, timing and transition values from `2026-08-31-v10-budget-estimates-audit.md`, plus the audit-only prime exception `prime-activity-recycle-2024`, and the two fiscal-promotion estimates above: these are exactly 18 newly audited entries.

Migrate every retained V9 option before it can enter `SCENARIO_V10_CATALOGUE`. Each retained nonzero option receives a separate named carry-forward estimate that preserves its V9 delta, annual/once nature and timing; uses the decision's existing institutional `sourceKeys`; records a nonempty scope, a nonnegative base amount and `estimateStatus: "scenario"`; and uses a stable key that never starts with `legacy:`. Eight annual carry-forward options retain their exact historical `after_decisions` thresholds: `etendre-le-dedoublement-des-classes-au-cm1:adopt` (+3), `ouvrir-200-000-places-de-creche:adopt` (+4), `cheque-education-par-eleve:adopt` (+3), `supprimer-le-financement-public-du-prive:adopt` (+2), `generaliser-le-service-national-universel:adopt` (+4), `autonomie-complete-des-etablissements:adopt` (+3), `ne-pas-remplacer-un-depart-administratif-sur:adopt` (+3) and `supprimer-les-departements:adopt` (+3). `regle-d-or-constitutionnelle:adopt` is the sole audited V10 topological adaptation: its explicit published `BudgetProfile` changes +3 to +1 so rank 71 activates at rank 72; the scheduler never substitutes the option horizon for a sourced profile. Every other V10 profile using `after_decisions` is rejected. It is forbidden for each of the 18 audited additions and both promotions, which remain `immediate` or `mandate_year`. Each retained V9 `adopt` whose `budgetDelta === 0` and whose `transitionFlows.length === 0` becomes a strict-null profile with no estimate, flow or scope. The sole neutral exception is the named prime-activity audit profile described below. Carry-forward entries must not silently claim the 18 newly audited scopes. The catalogue test iterates every non-null key and proves that it resolves, and asserts that every neutral option other than the prime exception is strict-null; the registry test proves there are no unreferenced entries and no `legacy:` key. Thus the 18 count above names only the newly audited additions, not the complete registry size.

Set `remplacer-prime-activite-prelevements-travail:adopt` to its audit key with `runRateMillions: 0`, `runRateTiming: null`, `transitionFlows: []`, `exclusiveScopeKeys: []` and the audited 10 300/10 300 reconciliation. Keep `unifier-ir-csg-bareme-continu:adopt` and `supprimer-subventions-directes-entreprises:adopt` selectable with the strict null profile (`estimateKey: null`, `exclusiveScopeKeys: []`), without `BudgetEstimate`; they claim no scope while their gain is null. Give every `keep` option the strict null profile. Delete the `fourteen` EPR2 alternative; retain its two internal IDs as `adopt` and `keep`, with visible labels `Engager six EPR2` and `Ne pas engager de nouvel EPR2`. Confirm source URLs point to the Senate PLF 2026 report, BOFiP restaurant VAT and the 2026 Voies et moyens table.
...
Convert `runRateTiming.immediate` to the selecting decision and `mandate_year` to `decisionCountAtMandateYearEnd`. Eight named carry-forward V9 profiles in Step 2 retain `after_decisions` at their historical threshold; the explicit V10 `regle-d-or-constitutionnelle:adopt` profile alone is adapted from +3 to +1 at rank 71 so it activates once at rank 72. The scheduler follows `BudgetProfile` only, and this constrained set remains unavailable to all other V10 profiles. Materialize each transition flow once under its declared globally unique causal ID; annual checkpoints include only flows first due since the preceding checkpoint while the recurring run rate persists. `reverseDecisionConsequences(state, decisionId)` removes queued events, active promises and unmaterialized transition flows sourced by that decision, retains ledger entries already materialized, and changes its record to `reversed`.
