...
Convert `runRateTiming.immediate` to the selecting decision and `mandate_year` to `decisionCountAtMandateYearEnd`. The nine named carry-forward V9 profiles in Step 2 retain `after_decisions` as a constrained compatibility exception and activate exactly once at their stated threshold; it remains unavailable to all other V10 profiles. Materialize each transition flow once under its declared globally unique causal ID; annual checkpoints include only flows first due since the preceding checkpoint while the recurring run rate persists. `reverseDecisionConsequences(state, decisionId)` removes queued events, active promises and unmaterialized transition flows sourced by that decision, retains ledger entries already materialized, and changes its record to `reversed`.
