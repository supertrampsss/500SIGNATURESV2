...
**Goal:** Make the 60-decision campaign fast to scan on mobile by exposing only the choice, budget, main effect and risk before details.

**Architecture:** Introduce a small presentation model between campaign state and HTML. Render one compact layer by default, move mechanisms and evidence into an accessible drawer, show only changed dashboard metrics, and explicitly restore scroll and focus after navigation.
...
- Campaign header uses at most two visual rows.
- Public title size is 30 to 32 px on mobile.
- Closed option cards show label, budget, one principal impact and one risk only.
- No large decision illustration is shown on mobile.
...
test("une carte fermée ne retient qu'un impact principal", () => {
  ...
  assert.equal(view.primaryImpact.key, "annualBalance");
  assert.equal(view.secondaryImpactCount, 2);
});
...
export function compactOption(option: DecisionOption): CompactOptionView {
  ...
  const primaryImpact = indicatorEffects[0];
  if (!primaryImpact) throw new Error(`Option without visible indicator effect: ${option.id}`);
  return {
    id: option.id,
    label: option.label,
    budget: indicatorEffects.find(({ key }) => key === "annualBalance"),
    primaryImpact,
    secondaryImpactCount: Math.max(0, indicatorEffects.length - 1),
    risk: option.contributors[0] ?? option.mechanism,
  };
}
```
...
Render one `<dialog id="simulateur-v3-details">` after the option list. Populate it from the chosen option rather than duplicating hidden detail content for every option.
...
### Task 3: Restore focus and scroll on every new dossier
...
### Task 4: Implement the mobile density rules
...
assert.match(css, /\.simulateur-v3__decision-title[\s\S]*font-size:\s*clamp\(1\.875rem,.*2rem\)/);
...
@media (max-width: 600px) {
  ...
  .simulateur-v3__decision-title { font-size: clamp(1.875rem, 8vw, 2rem); line-height: 1.02; }
  ...
  .simulateur-v3__option { min-height: 0; padding: 0.9rem; gap: 0.45rem; }
...
### Task 5: Group the journal by chapter and mandate year
...
Start the existing preview command, capture the intro, first dossier, selected option, result, crisis and journal at both sizes, and store the report under `docs/verification/2026-08-30-v3-compact/verification-report.md`. The report must record viewport, route, state and any deviation.
