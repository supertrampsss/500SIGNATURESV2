...
async function begin(page,mode,info){await page.goto(HOME);await activate(page.getByRole('button',{name:mode==='municipal'?/Gouverner une ville/:/Gouverner la France/}),info);await expect(page.getByRole('heading',{level:1})).toContainText('Tout commence');await activate(page.getByRole('button',{name:/Ouvrir le premier dossier/}),info);}
async function choose(page,info){await activate(page.locator('[data-action="choose"]:not([disabled])').first(),info);await expect(page.locator('.resolution')).toBeVisible();await noOverflow(page);}
for(const mode of ['municipal','national'])test(`${mode}: complete touch campaign, sharing and replay`,async({page},info)=>{
 ...
 for(let year=0;year<(mode==='municipal'?6:5);year++){
  ...
  await choose(page,info);await activate(page.locator('[data-action="next"]'),info);
 }
 ...
 await activate(page.getByRole('button',{name:'Rejouer le même défi',exact:true}),info);await expect(page.locator('.briefing')).toBeVisible();
});
...
test('territory, sandbox and reduced motion keep the saved mandate intact',async({page},info)=>{
 await page.emulateMedia({reducedMotion:'reduce'});await begin(page,'national',info);await choose(page,info);await activate(page.locator('[data-action="next"]'),info);
...
test('challenge URL is consumed and clipboard failure has an accessible fallback',async({page},info)=>{
 ...
 await page.goto(HOME+'?mode=municipal&v=2&ambition=services&seed=42');await activate(page.getByRole('button',{name:/Ouvrir le premier dossier/}),info);await choose(page,info);expect(new URL(page.url()).search).toBe('');
 await activate(page.getByRole('button',{name:'Partager cette décision',exact:true}),info);await activate(page.getByRole('button',{name:'Copier le lien',exact:true}),info);await expect(page.getByRole('textbox',{name:'Lien à copier'})).toBeFocused();await expect(page.getByRole('textbox')).toHaveValue(/#dilemma=/);await noOverflow(page);
