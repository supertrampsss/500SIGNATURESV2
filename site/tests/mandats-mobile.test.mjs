...
test('territory, sandbox and reduced motion keep the saved mandate intact',async({page},info)=>{
 ...
 await activate(page.getByRole('button',{name:'Ouvrir le bilan',exact:true}),info);await expect(page.getByRole('region',{name:'Les indicateurs du jeu',exact:true})).toContainText('Confiance');await expect(page.getByRole('region',{name:'Les indicateurs du jeu',exact:true})).toContainText('Patrimoine');await noOverflow(page);
