...
test('opt-in offline preparation survives network loss with the cinematic game art',async({page,context},info)=>{
 ...
 await context.setOffline(true);await page.goto(HOME);await activate(page.getByRole('button',{name:/Reprendre/}),info);await expect(page.locator('.dossier h1')).toBeVisible();await choose(page,info);await expect.poll(()=>page.locator('[data-cinema-art]').evaluate(image=>image.complete&&image.naturalWidth>0)).toBe(true);await activate(page.getByRole('button',{name:'Bilan',exact:true}),info);await expect.poll(()=>page.locator('.cinema-review__landscape img').evaluateAll(images=>images.length>0&&images.every(image=>image.complete&&image.naturalWidth>0))).toBe(true);await context.setOffline(false);
});
...
test('cinematic art keeps one scene image across decisions and offers an accessible territory', async ({page}, info) => {
 ...
 await choose(page,info);
 const secondYearScene=page.locator('[data-cinema-art]');
 ...
 await activate(page.getByRole('button',{name:/Espaces ruraux/}),info);
