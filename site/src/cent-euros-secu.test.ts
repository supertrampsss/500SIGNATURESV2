...
test("le bloc refuse le fléchage et annonce la moitié qui manque", () => {
  const html = plat(rendu(territoire(SERIES), CATALOGUE));
  ...
  assert.match(html, /ne publie pas encore la ventilation de ces recettes/);
  assert.match(html, /que le côté dépenses/);
