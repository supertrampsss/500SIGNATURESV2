...
test("le catalogue déclare exactement 47 trajectoires et 24 missions uniques", () => {
  assert.equal(RECETTES_TENDANCES.length, 47);
  ...
  assert.equal(19 + 10 + ids.length, 100);
  for (const id of [
    "recettes-non-fiscales",
    "pib-habitant",
    "inflation-insee",
    "creations-entreprises-nombre",
    "mariages",
    "pacs",
    "nuptialite",
    "vols-armes",
    ...
    assert.equal(ids.includes(id), false, `${id} doit être remplacé`);
  }
});
