...
export function insightChart(insight:Insight,catalogue:Indicateur[],data:Territoire['series']):string {
  ...
  const unit=indexed?`Évolution depuis ${common}`:unitRaw==='EUR' ? scale===1e9?'Md€':scale===1e6?'M€':'€' : UNITS[unitRaw]??unitRaw.replaceAll('_',' ');
  return timeChart({title:indexed?'Impôts locaux et dépenses : leur évolution':comparable.length>1?insight.surtitre:comparable[0].libelle,periodLabel:cohort?'Génération de naissance':undefined,description:indexed&&common?`Variation en pourcentage depuis ${common}.`: '',unit,series:comparable.map(i=>({name:i.libelle,pointsOnly:cohort,values:Object.fromEntries(Object.entries(data[i.id]).filter(([y])=>!indexed||!!common&&y>=common).map(([y,v])=>[y,indexed&&common?(v/data[i.id][common]-1)*100:v/scale]))})),format:v=>indexed?`${v>0?'+':''}${v.toLocaleString('fr-FR',{maximumFractionDigits:1})} %`:`${v.toLocaleString('fr-FR',{maximumSignificantDigits:4})}${unit?' '+unit:''}`});
}
