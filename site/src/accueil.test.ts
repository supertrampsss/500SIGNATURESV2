...
test("24. la bande compte les indicateurs qu'elle reçoit et cite ses producteurs sans logo", () => {
  ...
  assert.equal([...html.matchAll(/href="\/sources\/"/g)].length, 2);
  assert.ok(!html.includes('/bilan#methode-sources'), "un lien de confiance mène encore au bilan");
});
...
test("26. France, territoires et dossiers précèdent Mandats", () => {
  const html = page();
  const france = html.indexOf("Les comptes<br>de la France");
  const territoires = html.indexOf("Votre<br>territoire");
  const dossiers = html.indexOf("Les<br>dossiers");
...
test("27. l'accueil reste court : trois portes, trois questions, confiance, puis Mandats", () => {
  ...
  assert.ok(html.indexOf("Quelques questions à ouvrir.") < html.indexOf("Sources officielles"));
...
test("28. la promesse d'accueil garde un seul CTA principal", () => {
  const html = page();
  assert.ok(html.includes("Comprendre aujourd’hui"));
  assert.ok(html.includes("pour mieux agir demain."));
  assert.ok(html.includes("Comprendre la France"));
