...
import type { Choice, Domain, Effect, Game, Mode } from "../types.ts";
...
export function start(mode: Mode, seed = 42): Game {
  if (!Object.hasOwn(DOMAINS, mode) || !Number.isInteger(seed) || seed < 0 || seed > 9999) throw new Error("Scénario invalide.");
  return { version: 1, mode, seed, turn: 0, ...DOMAINS[mode].initial(), pending: [], history: [], choices: [] };
}
...
export function score(g: Game) {
  const dimensions = { finances: DOMAINS[g.mode].sustainability(g), services: g.metrics.services, cohesion: g.metrics.cohesion, resilience: (g.metrics.resilience + g.metrics.assets) / 2 };
  const terminalPenalty = g.pending.filter(p => (p.effect.operating ?? 0) > 0).reduce((sum, p) => sum + Math.min(5, p.effect.operating ?? 0), 0);
  const total = Math.round(clamp(Object.values(dimensions).reduce((a, b) => a + b, 0) / 4 - terminalPenalty));
  const labels: Record<string, string> = { finances: "Finances", services: "Services", cohesion: "Cohésion", resilience: "Résilience & patrimoine" };
  const ordered = Object.entries(dimensions).sort((a, b) => b[1] - a[1]);
  return { total, dimensions, terminalPenalty, strength: labels[ordered[0][0]], weakness: labels[ordered.at(-1)![0]], legacy: total >= 70 ? "Un héritage solide" : total >= 55 ? "Un équilibre à consolider" : "Un mandat sous tension" };
}
export function replay(mode: Mode, seed: number, ids: string[]): Game {
  if (!Object.hasOwn(DOMAINS, mode) || !Array.isArray(ids) || ids.length > DOMAINS[mode].turns) throw new Error("Journal trop long.");
  return ids.reduce((g, id) => decide(g, id), start(mode, seed));
}
