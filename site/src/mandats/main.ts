...
async function action(target: HTMLElement) {
  ...
  if (a === "mode") { if (g) track("mode_switched"); g = start(target.dataset.mode as Mode,42,"equilibre",target.dataset.mode === "national" ? 6 : 4); shared = false; inherited = false; screen = "mandate"; clearEntryLink(history); track("mode_selected"); if(g.mode === "national") {adopt(g);persist();track("onboarding_completed");} }
