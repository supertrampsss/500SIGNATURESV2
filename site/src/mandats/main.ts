...
async function action(target: HTMLElement) {
  ...
  if (a === "choose-cap" && g && screen === "mandate") { adopt(start(g.mode,g.seed,target.dataset.ambition as Ambition,g.version)); persist(); track("onboarding_completed"); render(); return; }
  ...
  if (a === "mode") { if (g) track("mode_switched"); g = start(target.dataset.mode as Mode); shared = false; inherited = false; screen = "mandate"; clearEntryLink(history); track("mode_selected"); }
