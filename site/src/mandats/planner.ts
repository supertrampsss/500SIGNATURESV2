...
import { choicesFor, decide, domainFor, calendarFor, startingGame, replayGame, score, isFinished } from "./engine.ts";
...
export function planner(reference: Game, ids: string[]) {
  ...
  const fieldList = d.dossiers.map((inheritedDossier, i) => {
    const before = p.states[i];
    const dossier = before ? (domainFor(before).dossiers[before.turn] ?? inheritedDossier) : inheritedDossier;
    const choices = before ? choicesFor(before) : reference.version >= 7 ? [] : dossier.choices;
    return `<label class="plan-field"><span>Dossier ${i + 1} · ${!before && reference.version >= 7 ? "Selon vos prochains choix" : e(dossier.category)}</span><select data-plan-year="${i}" ${!before ? "disabled" : ""}><option value="">Choisir un arbitrage</option>${choices.map(c => `<option value="${e(c.id)}" ${ids[i] === c.id ? "selected" : ""}>${e(choiceCopy(reference,dossier,c).title)}</option>`).join("")}</select></label>`;
  });
  const years = calendarFor(reference).years;
  const boundaries = reference.version >= 9 && reference.mode === "national" ? [0,6,12,18,24,30] : reference.version >= 3 ? (reference.mode === "municipal" ? [0,8,16,24,31,38,45] : [0,9,18,27,36,45]) : Array.from({length:years+1},(_,i)=>i);
  ...
  return `<article class="planner"><p class="eyebrow">ATELIER DE STRATÉGIE · MODÈLE V${reference.version}</p><h1 tabindex="-1">Et si vous aviez choisi autrement ?</h1><p>Rejouez le même scénario avec les mêmes règles. Votre partie sauvegardée reste intacte et chaque effet suit le modèle du jeu.</p><div class="planner-layout"><section><h2>Votre plan alternatif</h2><div class="plan-fields">${fields}</div><button class="button" data-action="plan-reset">Repartir de mon mandat</button>${p.error ? `<p class="choice-error" role="alert">Dossier ${p.game.turn + 1} : ${e(p.error)} Choisissez un autre arbitrage.</p>` : ''}</section><section><h2>Comparer à horizon identique</h2><p>Après ${p.commonYear} décisions. ${isFinished(p.game) ? "Le plan atteint sa fin simulée." : p.game.turn !== reference.turn ? `L’alternative compte ${p.game.turn} décisions, votre mandat ${reference.turn}. La comparaison s’arrête à leur dernier dossier commun.` : 'Même période et mêmes règles ; les conséquences dépendent de vos choix.'}</p><dl class="plan-comparison">${comparison}</dl>${isFinished(p.game) ? (p.game.politics?.ending && p.game.politics.ending.kind !== "term_complete" ? `<p class="plan-score">Plan interrompu après ${p.game.turn} décisions : ${e(p.game.politics.ending.title)}.</p>` : reference.version >= 9 ? `<p class="plan-score">Plan complet. Comparez les indicateurs ci-dessus et le journal annuel.</p>` : `<p class="plan-score">Bilan du plan complet <strong>${score(p.game).total}/100</strong></p>`) : `<p>Planifié : ${p.game.turn}/${d.turns} décisions. Complétez le plan pour voir son héritage.</p>`}<h3>Échéances et comptes du plan</h3><ol class="plan-years">${p.game.history.map(t => `<li><strong>Année ${t.year} : ${e(t.title)}</strong><span>${reference.version >= 3 && !t.closed ? "Prévision" : "Clôture"} · dette ${number(t.ledger.debt)} ${d.unit} · investissement ${number(t.ledger.investment)} ${d.unit}</span><small>${e(t.event)}</small></li>`).join('') || '<li>Aucun arbitrage planifié.</li>'}</ol><p class="scope">Résultats simulés, pas des prévisions. Cet atelier ne donne aucune recommandation de politique publique.</p></section></div></article>`;
}
