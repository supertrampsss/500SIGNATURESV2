...
export function start(mode: Mode, seed = 42, ambition: Ambition = "equilibre", version: 1 | 2 = 2): Game {
...
export function score(g: Game) {
  ...
  const dimensions = { finances: DOMAINS[g.mode].sustainability(g), services: g.metrics.services, cohesion: .7 * g.metrics.cohesion + .3 * g.metrics.trust, resilience: (g.metrics.resilience + g.metrics.assets) / 2 };
...
export function replay(mode: Mode, seed: number, ids: string[], version: 1 | 2 = 2, ambition: Ambition = "equilibre"): Game {
  if (!Object.hasOwn(DOMAINS, mode) || !Array.isArray(ids) || ids.length > DOMAINS[mode].turns) throw new Error("Journal trop long.");
  return ids.reduce((g, id) => decide(g, id), start(mode, seed, ambition, version));
}
