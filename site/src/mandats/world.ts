...
export function deliveryFeed(g: Game): string {
  ...
  return `<div class="delivery-feed">${recent.map(p => `<div class="delivery delivered"><span class="delivery-symbol">${icon("check")}</span><span><small>EN SERVICE CETTE ANNÉE</small><strong>${e(p.title)}</strong></span></div>`).join("")}${pending.map(p => `<div class="delivery"><span class="delivery-symbol" aria-hidden="true">${Math.max(1, p.due - calendarFor(g).completedYears)}</span><span><small>LIVRAISON · ANNÉE ${p.due}</small><strong>${e(p.title)}</strong></span></div>`).join("")}${allPending.length > 2 ? `<p class="delivery-empty">${allPending.length - 2} autres programmes dans le journal.</p>` : ""}${!recent.length && !pending.length ? '<p class="delivery-empty">Chaque décision prépare la suite du mandat.</p>' : ""}</div>`;
}
