...
export function nationalCommandPulse(game: Game): string {
  ...
  return `<dl class="national-command-pulse" aria-label="État du pays dans la simulation">${impact.length ? `<div class="national-decision-impact" aria-live="polite"><dt>Effet de la mesure</dt><dd>${impact.map(item => `<span data-direction="${item.direction}">${item.label}</span>`).join('')}</dd></div>` : ''}<div class="national-deficit" aria-live="polite"><dt>${deficit < -.000001 ? 'Excédent annuel' : 'Déficit annuel'}</dt><dd>${amount} <small>Md€</small><small class="deficit-context">Année ${year}${last?.closed ? ' clôturée' : ' · budget prévu'}</small><small class="deficit-context">Départ : ${number(initial)} Md€ · Objectif : 0</small></dd></div><div><dt>Services publics</dt><dd>${number(game.metrics.services)}<small>/100</small></dd></div></dl>`;
}
