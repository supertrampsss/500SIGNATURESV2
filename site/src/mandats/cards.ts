import { choicesFor, DOMAINS, replay, score } from "./engine.ts";
import { CARD_SIZES, challengeURL, escape as e, resultURL, shareText } from "./sharing.ts";
import { decode } from "./storage.ts";
...
export function cardModel(g: Game, kind: CardKind) {
  const d = DOMAINS[g.mode];
  if (kind === 'decision') {
    if (!g.turn) throw new Error('Prenez une décision avant de la partager.');
    const previous = replay(g.mode, g.seed, g.choices.slice(0, -1), g.version, g.ambition);
    const c = choicesFor(previous).find(c => c.id === g.choices.at(-1))!;
    const url = `${new URL('/mandats/', 'https://500signatures.fr').href}#dilemma=${encodeURIComponent(JSON.stringify({version:previous.version, mode:previous.mode, seed:previous.seed, choices:previous.choices, ...(previous.version === 2 ? {ambition:previous.ambition} : {})}))}`;
    return { label: `DÉCISION DE JEU · ANNÉE ${g.turn}`, title: c.title, fields: [['Coût du choix', c.cost], ['Effet annoncé', c.benefit], ['Compromis', c.sacrifice], ['Délai', c.delayed ? `Livraison en année ${previous.turn + c.delayed.after + 1}` : 'Effet immédiat, selon les règles']], url, alt: `Décision de jeu, ${d.place}, année ${g.turn}. ${c.title}. ${c.cost}. ${c.benefit}. Compromis : ${c.sacrifice}. ${c.delayed ? `Livraison en année ${previous.turn + c.delayed.after + 1}.` : ''} Le lien restitue les décisions antérieures pour rejouer ce dilemme. Simulation fictive v${g.version}.` };
  }
  if (kind === 'challenge') {
    return { label:'DÉFI JOUABLE · SANS VOS CHOIX', title:g.mode === 'municipal' ? 'Quel avenir pour Val-sur-Rive ?' : 'Quel cap pour le pays ?', fields: [['Votre mission', d.objectives[0]], ['Durée', d.duration], ['Priorité', g.version === 1 ? 'Équilibre, règles v1' : ambitionFor(g).label], ['Même point de départ', `Scénario ${g.seed} · ${d.turns} tours`]], url:challengeURL(g, 'https://500signatures.fr'), alt:`Défi de jeu, ${d.place}. ${d.objectives[0]}. ${d.duration}. Priorité : ${g.version === 1 ? 'Équilibre v1' : ambitionFor(g).label}. Scénario ${g.seed}, simulation fictive v${g.version}. Le lien ne contient aucune décision du joueur.` };
  }
  const s = score(g);
...
export function dilemmaFromHash(hash: string): Game | null {
  ...
  if (game.turn >= DOMAINS[game.mode].turns) throw new Error("Ce dilemme est déjà terminé.");
  return game;
}
...
export function cardSVG(g: Game, kind: CardKind, format: keyof typeof CARD_SIZES) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}"><title>${e(m.alt)}</title><rect width="${w}" height="${h}" fill="#09151b"/><rect x="30" y="30" width="${w-60}" height="${h-60}" rx="24" fill="#142e35" stroke="#50727b"/><path d="M${w-310} 30V150H${w-30}" fill="none" stroke="#e8c69b" opacity=".35"/>${label('MANDATS  /  500 SIGNATURES',72,97,25,'#e8c69b')}${label(m.label,72,145,19,'#bdd3d4')}${title}${fields}<path d="M72 ${h-139}H${w-72}" stroke="#50727b"/>${label('À vous de jouer : 500signatures.fr/mandats/',72,h-102,21,'#e8c69b')}${label(`${DOMAINS[g.mode].place} · Modèle v${g.version}`,72,h-69,18,'#bdd3d4')}${label('SIMULATION FICTIVE · AUCUNE PRÉDICTION',72,h-43,16,'#bdd3d4')}</svg>`;
}
