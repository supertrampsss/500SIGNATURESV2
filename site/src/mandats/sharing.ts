...
import { DOMAINS, score, start } from "./engine.ts";
...
export function challengeURL(g: Game, origin: string): string {
  ...
  if (g.version === 2) url.searchParams.set("ambition", g.ambition ?? "equilibre");
...
export function sharedResult(hash: string): Game | null {
  ...
  if (g.turn !== DOMAINS[g.mode].turns) throw new Error("Le résultat partagé n'est pas un mandat terminé.");
  return g;
}
export function challengeFromURL(url: URL): Game | null {
  ...
  if (version !== "1" && version !== "2") throw new Error("Version de défi inconnue.");
  const ambition = url.searchParams.get("ambition") ?? "equilibre";
  return start(mode as Mode, Number(seed), ambition as Ambition, Number(version) as 1 | 2);
}
export function shareText(g: Game): string {
  const s = score(g); const d = DOMAINS[g.mode];
  return `Mandats · ${d.place}. ${s.legacy} : ${s.total}/100. ${g.version === 2 ? `Priorité : ${ambitionFor(g).label}. ` : ""}Point fort : ${s.strength}. À améliorer : ${s.weakness}. Résultat de jeu, scénario fictif v${g.version}.`;
}
...
export function socialSVG(g: Game | null, format: keyof typeof CARD_SIZES = "landscape"): string {
  ...
  const title = g ? DOMAINS[g.mode].place : "Choisissez votre mandat";
  ...
  const labels = { finances: "Finances", services: "Services", cohesion: g?.version === 2 ? "Cohésion / confiance" : "Cohésion", resilience: "Résilience / patrimoine" };
  ...
  const mode = g ? `${g.mode === "municipal" ? "MANDAT MUNICIPAL" : "MANDAT NATIONAL"} · ${g.turn} TOURS${g.version === 2 ? ` · PRIORITÉ ${ambitionFor(g).short.toLocaleUpperCase("fr")}` : ""}` : "UN JEU DE STRATÉGIE PUBLIQUE";
