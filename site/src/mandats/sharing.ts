...
export function challengeFromURL(url: URL): Game | null {
  ...
  const version = url.searchParams.get("v") ?? (directFrance ? "6" : "1");
  if (version !== "1" && version !== "2" && version !== "3" && version !== "4" && version !== "5" && version !== "6") throw new Error("Version de défi inconnue.");
  const ambition = url.searchParams.get("ambition") ?? "equilibre";
  return start(mode as Mode, Number(seed), ambition as Ambition, Number(version) as 1 | 2 | 3 | 4 | 5 | 6);
}
