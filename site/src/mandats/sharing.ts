...
export function challengeFromURL(url: URL): Game | null {
  ...
  return start(mode as Mode, Number(seed));
}
export function shareText(g: Game): string {
  const s = score(g); const d = DOMAINS[g.mode];
  return `Mandats · ${d.place}. ${s.legacy} : ${s.total}/100. Point fort : ${s.strength}. À améliorer : ${s.weakness}. Résultat de jeu, scénario fictif v1.`;
}
...
export function socialSVG(g: Game | null, format: keyof typeof CARD_SIZES = "landscape"): string {
  ...
  const labels = { finances: "Finances", services: "Services", cohesion: "Cohésion", resilience: "Résilience / patrimoine" };
