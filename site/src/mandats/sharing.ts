...
export function socialSVG(g: Game | null, format: keyof typeof CARD_SIZES = "landscape"): string {
  ...
  const mode = g ? `${g.mode === "municipal" ? "MANDAT MUNICIPAL" : "MANDAT NATIONAL"} · ${g.turn} DÉCISIONS${g.version !== 1 ? ` · MISSION ${ambitionFor(g).short.toLocaleUpperCase("fr")}` : ""}` : "UN JEU DE STRATÉGIE PUBLIQUE";
