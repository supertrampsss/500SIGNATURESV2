...
export const V9_NATIONAL_YEAR_SLOTS = [6,6,6,6,6] as const;
export function calendarFor(g: Pick<Game,'version'|'mode'|'turn'>) {
  const slots = g.version >= 9 && g.mode === 'national'
    ? V9_NATIONAL_YEAR_SLOTS
    : g.version >= 3 ? YEAR_SLOTS[g.mode] : Array(g.mode === 'municipal' ? 6 : 5).fill(1) as number[];
