...
const THEMES: {id:FamilleInsight; titre:string; image:string; alt:string; repere:string; texte:string}[] = [
  {id:'budget',titre:'Dette et budget',image:'budget',alt:'Pièces de monnaie',repere:'interets-dette',texte:'Comment la dette finance les politiques publiques ?'},
  {id:'fiscalite',titre:'Fiscalité',image:'fiscalite',alt:'Calculatrice et documents',repere:'prelevements-obligatoires',texte:'Qui paie quoi ?'},
  ...
  {id:'generation',titre:'Retraites et générations',image:'retraites',alt:'Mains d’une personne âgée',repere:'cotisants-retraites',texte:'Un équilibre intergénérationnel.'},
  {id:'services',titre:'Niveau de vie et services publics',image:'services',alt:'Habitants dans un espace public',repere:'pauvrete',texte:'Des services publics pour tous.'},
...
function arbitrages(pays:Record<string,Territoire>,catalogue:Indicateur[]):string {
  const analyses=insightsFrance(pays.FR,catalogue,pays);
  const themes=[...THEMES];
  ...
  return `<section id="insights-france" class="fr-panel fr-arbitrages"><header class="fr-section-heading"><div><h2>Les arbitrages derrière les comptes</h2><p class="fr-subtitle">${themes.length===7?'Sept':themes.length} familles pour comprendre les choix publics</p></div><a href="/analyses/">Voir tous les dossiers</a></header><div class="fr-topics">${themes.map(t=>{
...
export function renduFrancePage(pays:Record<string,Territoire>,catalogue:Indicateur[],analyses:readonly Analyse[]=[], indexSources?:IndexSources):string {
  ...
  return `<div class="fr-page" data-france-design="maquette-complete-20260921">
    <section class="fr-hero" id="france-verdict"><img class="fr-hero-photo" src="/france/assemblee.jpg" alt="Façade de l'Assemblée nationale à Paris, surmontée du drapeau français" width="1280" height="850" fetchpriority="high"><div class="fr-hero-copy"><p class="fr-eyebrow">Comptes publics français · ${c.fin}</p><h1>Les comptes<br>de la France.</h1><p class="fr-hero-statement">La France ${c.emprunte>=0?'dépense':'encaisse'} ${nombre(Math.abs(c.emprunte)/1e9)} milliards<br class="fr-desktop-break"> d’euros de plus qu’elle ${c.emprunte>=0?'n’encaisse':'ne dépense'}.</p><p class="fr-hero-scope">État, collectivités et Sécurité sociale réunis.</p><a class="fr-button" href="/analyses/">Voir les dossiers</a></div><p class="fr-hero-note">Une démocratie<br>éclairée par<br>les chiffres.</p><a class="fr-photo-credit" href="#france-credits">Assemblée nationale, Paris</a></section>
