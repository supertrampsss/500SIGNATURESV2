...
test("V11 porte une copie visible complète et seulement des profils V10 ou nuls", () => {
  ...
  for (const decision of SCENARIO_V11_CATALOGUE.decisions) {
    ...
    for (const option of decision.options) {
      ...
      assert.equal(v10Profiles.has(JSON.stringify(option.budgetProfile)) || option.budgetProfile.runRateMillions === 0, true, option.id);
    }
  }
});
